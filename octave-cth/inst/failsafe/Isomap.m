function [Y, R, E, C] = Isomap(D, n_fcn, n_size, options)
...
%r2 = 1-corrcoef(reshape(real(L2_distance(Y.coords{di}, Y.coords{di})),N^2,1),D).^2; 
         r2 = 1-corr(reshape(real(L2_distance(Y.coords{di}, Y.coords{di})),N^2,1),D).^2;
...
%         plot(Y.coords{twod}(1,:), Y.coords{twod}(2,:), 'marker', 'o', 'color', [rand(1) rand(1) rand(1)]); 
         if (overlay == 1)
