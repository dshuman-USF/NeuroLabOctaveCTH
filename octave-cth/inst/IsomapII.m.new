function [Y, R, E, C] = IsomapII(D, n_fcn, n_size, options)
...
% Modified by dshuman to return C
     ...
     if n_fcn == 'k'
         ...
         for i=1:N
             ...
             if ((verbose == 1) && (rem(i,50) == 0)) 
                 disp([' Iteration: ' num2str(i) ' Estimated time to completion: ',num2str((N-i)*toc/k/60) ' minutes']); 
             end
         end
     elseif n_fcn == 'epsilon'
...
if (nl==N)
     opt.disp = 0; 
      % octave can give up and return an error when we use "LR".
      % matlab is our pal, and changes this to "LA" down in its eigs function 
      % if this error occurs, so if we get an error, we do that here.
   try
     [vec, val] = eigs(-.5*(D.^2 - sum(D.^2)'*ones(1,N)/N - ones(N,1)*sum(D.^2)/N + sum(sum(D.^2))/(N^2)), max(dims), 'lr', opt); 
   catch
     [vec, val] = eigs(-.5*(D.^2 - sum(D.^2)'*ones(1,N)/N - ones(N,1)*sum(D.^2)/N + sum(sum(D.^2))/(N^2)), max(dims), 'la', opt); 
   end
...
% original - use corr instead of corrcoef
%     r2 = 1-corrcoef([reshape(real(L2_distance(Y.coords{di}, Y.coords{di}(:,land_ind))),N*nl,1) D]).^2; 
     r2 = 1-corr(reshape(real(L2_distance(Y.coords{di}, Y.coords{di}(:,land_ind))),N*nl,1),D).^2; 
     % matlab returns a matrix, octave returns a scalar.
     % matlab r2(2,1) == octave r2
     % original R(di) = r2(2,1); 
     R(di) = r2; 
     if (verbose == 1)
